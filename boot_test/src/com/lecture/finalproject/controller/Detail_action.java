...
public class Detail_action extends HttpServlet {
	...
	{  
		...
		ModelLocation location = db.getLocationOne(Integer.parseInt(postNum));
		...
		if(isLogin){
			...
			int visitState = db.getVisitState(Integer.parseInt(postNum), Long.toString(twitterUser.getId()));

			request.setAttribute("locationObject", location);
			request.setAttribute("likeState", likeState);
			request.setAttribute("visitState", visitState);
